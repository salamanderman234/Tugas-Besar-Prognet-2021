        ...
        <div class="btn-group border d-flex align-items-center ps-2 border-dark rounded-3">
            ...
            <select name="tahun_ajaran" id="tahun_ajaran" class="form-select rounded-0 ms-0">
              @if (!is_null($tahun_ajaran_sekarang))
                ...
                {{$semester = $tahun_ajaran_sekarang['value']}}
              @endif
              @foreach ($tahun_ajarans as $tahun_ajaran)
                ...
                @if (is_null($tahun_ajaran_sekarang) && $loop->index+1 == count($tahun_ajarans))
...
<script>
  $(document).ready(function(){
      ...
      var url = '{{ route('cari_krs',['semester'=> "nilai"])}}'.replace('nilai',$('#tahun_ajaran').val());
      tabel_krs(url);
      $('#tahun_ajaran').on('change',function(){
        $('tbody').empty();
        var url_ajax = '{{ route('cari_krs',['semester'=> "nilai"])}}'.replace('nilai',$('#tahun_ajaran').val());
        tabel_krs(url_ajax);
      });
  });
</script>
